...
func NewManager() *Manage {
	return &Manage{TopicList: &pb.TopicList{
...
func (mgr *Manage) Add(topic string) (*pb.Topic, error) {
	t, ok := mgr.List[topic]
	if ok {
...
func (mgr *Manage) Get(topic string) (*pb.Topic, error) {
	mgr.lock.RLock()
...
func (mgr *Manage) UpData(topic *pb.Topic) error {
...
func (mgr *Manage) AddSequence(topic string) (*pb.Topic, error) {
	_, err := mgr.Get(topic)
	if err != nil {
		return nil, err
	}
